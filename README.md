...
/* Zusätzliche Themes für Level 34–200. */
...
function getWeaponMuzzle(weaponName){
  const lengths={PISTOLE:32,FALCON:40,VECTOR:49,SPECTRE:47,RAVEN:61,PHANTOM:57,BREACH:59,NIGHTFALL:83,VANGUARD:71,SHADOW:84,HAMMER:62,FURY:77,VOLT:79,TITAN:62,"D-WOLF":86,RAILGUN:108};
...
function shoot(){
  ...
  for(let i=0;i<w.pellets;i++){
    ...
    bullets.push({x:mx,y:my,vx:Math.cos(angle)*w.speed,vy:Math.sin(angle)*w.speed,life:1.15,damage:w.damage,explosive:w.explosive||0,shock:!!w.shock,railgun:!!w.railgun});
  }
  flash=.07;
...
function updateEnemy(e,dt){
  ...
  if(e.dead){
    e.fall=Math.min(1,(e.fall||0)+dt*2.35);
    return;
  }
...
function updateHUD(){
  ...
  document.getElementById("ammoText").textContent=player.reload>0?"...":player.ammo+"/"+player.reserve;
...
function drawHuman(x,y,dir,enemy,camo=false,phase=0,walking=false,weaponName="PISTOLE",recoil=0,deathPose=0,drawShadow=true){
  ...
  const pants=outfit==="DIAMONDREACTIVE"&&!enemy?"#d9fbff":outfit==="EXECUTIVE"&&!enemy?"#17191a":outfit==="URBAN"&&!enemy?"#4d575a":outfit==="DESERT"&&!enemy?"#75664b":outfit==="RECON"&&!enemy?"#26352d":outfit==="BLACKOPS"&&!enemy?"#111416":outfit==="D-WOLF"&&!enemy?"#6f6047":"#1a2022";
  ...
  */
  ...
  if(weaponName==="PISTOLE"||weaponName==="FALCON"){
...
function drawDeadEnemy(e,x,progress){
  const kneel=Math.min(1,progress/.34);
  const fall=Math.max(0,Math.min(1,(progress-.30)/.70));
  const lean=e.deathDir||1;

  /* Der Schatten verschwindet beim Ableben und bleibt nicht unter dem
     umgefallenen Körper kleben. */
  ctx.save();
  if(fall>0){
    ctx.translate(x,e.y);
    ctx.rotate((e.fallAngle||1.35)*fall);
    ctx.translate(-x,-e.y);
  }

  /* Während der Kniephase sitzt der Körper sichtbar tiefer. */
  ctx.save();
  const ky=e.y + kneel*14;
  ctx.translate(0,ky-e.y);
  ctx.scale(1,1-kneel*.12);
  drawHuman(x,e.y,e.dir,true,false,e.walkPhase,false,"NONE",0,kneel,false);
  ctx.restore();
  ctx.restore();

  /* Ab der Fallphase liegt die Waffe getrennt vom Körper am Boden. */
  if(e.weaponDropped && fall>.12){
    const dropX=x + e.dir*(18+fall*26);
    const dropY=e.y+5+fall*12;
    ctx.save();
    ctx.translate(dropX,dropY);
    ctx.rotate(lean*(.45+fall*1.15));
    ctx.globalAlpha=Math.max(.25,1-fall*.15);
    ctx.fillStyle="#101416";
    ctx.fillRect(-3,-3,32,5);
    ctx.fillStyle="#596468";
    ctx.fillRect(7,-5,13,4);
    ctx.fillStyle="#171c1d";
    ctx.fillRect(3,1,7,10);
    ctx.restore();
  }
}
